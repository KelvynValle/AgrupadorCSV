...
namespace ClusterCSV.Graficos
{
    ...
    class Dendrograma : Diagrama
    {
        ...
        public void definirSequencia(int quantidade, List<Relacao> relacoes, tipoSequencia tipoSequencia, int[] sequenciaCSV)
        {
            switch (tipoSequencia)
            {
                case tipoSequencia.auto:
                    int contador = 0;
                    ...
                    for (int i = 0; i < relacoes.Count; i++)
                    {
                        ...
                        {
                            amostrasOrdenadas[contador] = (int)((RelacaoInicial)((RelacaoNoNo)relacoes[i]).no1).no1;
                            contador++;
                            amostrasOrdenadas[contador] = (int)((RelacaoInicial)((RelacaoNoNo)relacoes[i]).no2).no1;
                            contador++;
                        }
                        else if (relacoes[i] is RelacaoGrupoNo)
                        {
                            amostrasOrdenadas[contador] = (int)((RelacaoInicial)((RelacaoGrupoNo)relacoes[i]).no2).no1;
                            contador++;
                        }
                    }
                    break;
                case tipoSequencia.csv:
                    amostrasOrdenadas = (int[])sequenciaCSV.Clone();
                    break;
                case tipoSequencia.original:
                    amostrasOrdenadas = new int[quantidade];
                    for(int i = 0; i < quantidade; i++)
                    {
                        amostrasOrdenadas[i] = i;
                    }
                    break;
            }
        ...
        {
            desenharTexto(new PointF((float)Array.IndexOf(amostrasOrdenadas, inicio.no1), 0.0f), _amostras[(int)inicio.no1]);
        ...
        {
            int min = new int[] { (int)((RelacaoInicial)relacao.no1).no1, (int)((RelacaoInicial)relacao.no2).no1 }.Min();
            int max = min < (int)((RelacaoInicial)relacao.no1).no1 ? (int)((RelacaoInicial)relacao.no1).no1 : (int)((RelacaoInicial)relacao.no2).no1;
        ...
        public Bitmap desenharPartindoDeRelacoes(List<Relacao> relacoes, int quantidade, tipoSequencia tipoSequencia, int[] sequencia)
        {
            definirSequencia(quantidade, relacoes, tipoSequencia, sequencia);
            for (int i = 0; i < relacoes.Count; i++)
            {
